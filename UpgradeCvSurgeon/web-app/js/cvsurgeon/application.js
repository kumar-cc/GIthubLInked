...
function getNavElement(current) {
    var currentText = $.trim(current.find(">a").text()),
        ...
        element = "";
    element += "<li><a href='" + current.find(">a").attr("href") + "' class='"+current.find(">a").attr("class")+"'>" + currentText + "</a>";
...
$(document).ready(function () {
    ...
    if ($(".username-check").length > 0) {
        ...
        $(".username-check").change(function (e) {
            ...
            $.post("/editAccountDetails/checkUserNameAvailability", {username: $this.val()}, function (e) {
                if (e.available == "true") {
    ...
    if ($(".email-check").length > 0) {
        ...
        $(".email-check").keyup(function (e) {
            var $this = $(this);
            var input=$this.val();
            if(input) {
                $this.next('span').html("<i class='icon-spinner icon-spin'></i> Checking availability...");
                $.post("/register/checkEmailAvailability", {email: $this.val()}, function (e) {
                    if (e.available == "true") {
                        $this.next('span').html("<i class='icon-ok'></i> This email user ID is available!");
                        $this.next('#msg').removeClass("error").addClass("success");
                    } else if (e.available == "validEmail") {
                        $this.next('span').html("<i class='icon-remove'></i> Please enter a valid email address");
                        $this.next('#msg').removeClass("success").addClass("error");
                    } else {
                        $this.next('span').html("<i class='icon-remove'></i>This email address is already applied to a Resume Surgery account and therefore is not available.");
                        $this.next('#msg').removeClass("success").addClass("error");
                    }
                }, "json");
            } else{
        ...
        $(".email-check-force").click(function (e) {
            e.preventDefault();
            $(".email-check").trigger("change");
        });
...
// set resize handler to corret height
    ...
    $(".theme-colors > li > span").hover(function (e) {
        var $el = $(this),
            body = $('body');
        body.attr("class", "").addClass("theme-" + $el.attr("class")).attr("data-theme", "theme-" + $el.attr("class"));
    },function () {
        var $el = $(this),
            body = $('body');
