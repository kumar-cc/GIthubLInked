...
function notificationAlert(title, text){
    $.gritter.add({
    ...
    if($('.calendar').length > 0)
    {
        calendar=$('.calendar').fullCalendar({
            ...
            select: function(start, end, allDay) {
                var modalElement=$('#modal-2');
                modalElement.find("#calendarId").val($("#calenarSelect").val());
                modalElement.find("#start").val(start);
                modalElement.find("#end").val(end);
                modalElement.modal("show");
            },
            eventClick: function(event) {
                $.ajax({
                    url:"/calendar/show",
                    data:"calendarId="+jQuery("#calenarSelect").val()+"&eventId="+event.id,
                    success:function(data){
                        var modalElement=$('#eventDetails');
            ...
            eventDrop: function(event, delta) {
                $.ajax({
                    ...
                    data:"calendarId="+jQuery("#calenarSelect").val()+"&eventId="+event.id+"&end="+(event.end+delta)+"&start="+(event.start+delta) ,

                    success:function(data){
                      if(data.status=='error'){
                          notificationAlert('Calendar Notification', data.message);
                          event.start=new Date(event.start.getFullYear(), event.start.getMonth(), event.start.getDate()-(delta)) ;
                          if(event.end){
                              event.end=new Date(event.end.getFullYear(), event.end.getMonth(), event.end.getDate()-(delta));
                          }
...
/*
                alert(event.title + ' was moved ' + delta + ' days\n' +
                    '(should probably update your'+event.id+' database)');*/
            },
            header: {
                left: '',
...
function addNewEvent(){
    if(jQuery("#summary1").val()){
        $.ajax({
            url:'/calendar/addNewEvent',
            data:$("#addEvent").serialize(),
            success:function(data){
                if(data.status=='success'){
                    calendar.fullCalendar('renderEvent', {
                        title: jQuery("#summary1").val(),
                        start: jQuery("#start").val(),
                        end: jQuery("#end").val()
                    }, true);
                } else{
                    notificationAlert('Calendar Event', data.message);
                }
